...
public class familyTree {
	...
	private class Generation {
		...
		public boolean areChildren(ArrayList<Node> parent) { // checks whether the current generation are the children
	...
	// Priority queue class that holds any nodes whose parents/children are not yet
	// in the family tree
	private class Queue {
		...
		public ArrayList<Node> dequeue(String parent) { // removes node(s) whose parent matches the input string and
	...
	private Queue queue; // queue which temporarily holds nodes which have not yet been added to the
							// overall family tree
	...
	/*
	 * fillTree represents the core method that organises the family tree structure
	 * into successive generations.
	 * 
	 * Initially, it reads in the first node and fills it into the tree, then for
	 * every other node read from the file, it performs the following two:
	 * 
	 * 1) it checks whether the read node is the main parent in the family tree (i.e
	 * has a NULL parent). If so, it adds it to the root of the tree, then
	 * moves on to read the next node in the file.
	 * Else, it performs step two.
	 * 2) it checks whether any of the current nodes in the tree is a parent/child
	 * of the read node. If so, it adds it in its respective generation
	 * position in the tree. (This procedure optimises the speed in which nodes are
	 * added into the tree instead of just filling every node (except
	 * for the root node) into a queue)
	 * If not, it stores the node in the priority queue and reads the next node from
	 * the file.
	 * 
	 * When all nodes have been read from the file, every node that is currently in
	 * the tree (starting from the root node, moving breadth first) checks
	 * whether it has any children stored in the queue. If so, all its children are
	 * removed from the queue and added to the tree IN AN ORDERLY MANNER.
	 * A final check is done to confirm that there are no nodes remaining in the
	 * queue. The tree is now filled.
	 */
	public void fillTree(String filename) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				// fill the first read node into the tree regardless of whether it is the root
				// or not. (This procedure speeds up the fillTree process)
				boolean found = false;
				if (genTree.size() == 0 || names[0].equals("NULL")) {
					...
					for (int i = 0; !found && i < genTree.size(); i++) {
						for (Node node : genTree.get(i).nodes) { // check if the read node is a parent/child of any node
																	// in the tree (check generation after generation)
							if (names[1].equals(node.parent) && i == 0) { // check if it is a parent
								Generation gen = new Generation(names[1], names[0]);
								genTree.add(0, gen);
								found = true;
								break;
							} else if (names[1].equals(node.parent) && i > 0
								...
								found = true;
								break;
							} else if (names[0].equals(node.name) && i + 1 == genTree.size()) { // now check if it is a
																								// child of any node
								Generation gen = new Generation(names[1], names[0]);
								genTree.add(gen);
								found = true;
